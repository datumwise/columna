...
The destination, stated in full, is a *pair*. An anchor names a coordinate signature; it does not by itself determine which points inhabit it — two columns addressed by the same coordinates can live on different declared **universes** (the expected hourly readings and the alerts that happened to fire are both at `{sensor, hour}`, and are not about the same points). Every ascription is therefore implicitly `expr @ (a, U)`: the anchor names the coordinates, the universe defines the points. In the common case `U` is never written — it is supplied by the **consequence rule** (the contributing columns' covered universes, brought to `a`) — and it is always *disclosed*: the resolved `(anchor, universe)` of every series rides in the result's annotation. Where the writer wants a different destination — most often a reporting grid over a declared spine rather than the observed event projection — the universe is pinned explicitly with `ON`:

```
... AT {customer, month} ON customer_month_grid
```

Default by disclosure, pin by syntax — the same discipline as the Manifold version of Chapter 1.3, one level deeper. The choice is consequential and the framework treats it as such: over a declared (spine- or registry-based) universe, an empty bucket's `SUM` and `COUNT` are **true zeros** — counting nothing is a defined act — while its order statistics and means are absent; over an event-projection universe, the empty bucket is not a point at all, and nothing is rendered. (The full universe model — bases, declarations, co-universality — is the framework manual's; this manual specifies only what the query language exposes: the `ON` form, the consequence rule, and the disclosure. Beneath this surface, Columna's universe carries a population-consistency layer — the columns sharing a universe are reconciled against its point set, and a reduced total is path-independent across the anchorings that reach a given grain; this is semantics under the syntax, and does not change the forms above.)
...
- **Mule without explicit input anchor.** A mule reducer whose answer depends on an input grain that has not been named. What to clarify: the `@ {anchor}`.
...
- **No data.** The rows are not there: an empty realized anchor, disjoint filtered restrictions, a requested combination the bound data cannot support.
...
- **B-anchor crossing** (`critical`). A reduction coarsening across a blocked family — summing a stock over time, re-summing a broadcast value along its broadcast axis. Served with the crossing named, the double-count or replication factor quantified, and the applicable alternative reducer as remedy (Chapter 5.3).
...
Query-level: `FROM`, `VERSION`, `SELECT`, `AS`, `AT`, `ON`, `WHERE`, `HAVING`, `ORDER`, `BY`, `LIMIT`, `PER`, `WITH`, `ASC`, `DESC`, `IN`.
