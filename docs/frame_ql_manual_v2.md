...
4. **Version stamps.** The manual now documents columna-core **0.14.0** as implemented (preface, harness note, Chapters 2.9, Appendix B), and cites wire contract **`"2"`**. The retired terse `@`-fragment (Appendix D) and the grow-by-ruling surfaces are unchanged.
...
Two further expression forms are documented as the language the envelope **grows into by ruling** (ADR-035 D1); they are *not* part of the shipped 0.14.0 envelope, and the ratified grammar spec keeps them roadmap (they are grammar the engine does not yet have). They enter the language when ruled in, not by accretion.
...
*End of the Frame-QL Manual, Second Edition (a mechanically-reconciled continuation of the First Edition — synced to columna-core 0.14.0 / 0.13.4 and wire contract `"2"`; itself a renamed continuation of the Coframe-QL Manual, Third Edition, reconciled to the shipped envelope grammar, ADR-035).*
