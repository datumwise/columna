...
parser (`columna_core.envelope.parse_statement`, columna-core 0.17.0) by `docs/tools/check_manual_frameql.py`,
...
Two further expression forms are documented as the language the envelope **grows into by ruling** (ADR-035 D1); they are *not* part of the shipped envelope (columna-core 0.17.0), and the ratified grammar spec keeps them roadmap (they are grammar the engine does not yet have). They enter the language when ruled in, not by accretion.
