...
<!-- Reconciliation pass (Fifth Edition) against ADR-031 D1–D15, kept consistent with the framework manual Sixth Edition. Substantive changes, each marked inline: Ch 4.3 — relating along a roll-up edge is transport in the engine; the backend delivers columns, never combines them (D1/D9). Ch 19.2 — the membership aggregation over an unresolved many-to-many moved from "Serve, with disclosures" to "Clarify," on the determinacy principle (D10), superseding the ADR-020/025 default-to-membership reading; the B-anchor crossing remains serve-and-disclose. Ch 22.5 — the bare aggregate-across is a clarification; membership served only when chosen; allocation [Pro] (D6/D15). Appendix A — allocation bridges marked Pro (not Core), consistent with the registry chapter; <!-- tier-history: the two [Pro] classifications recorded in this changelog line were RETIRED on 2026-08-31 (topology record §17.5); the body text they describe now reads [ROADMAP]. The line is preserved as the record of what the Fifth-Edition pass did. --> the connector-delivers-never-combines contract stated (D1). Appendix B — transport, fan-out, and the M:N→Clarify reclassification added; watermark advanced to ADR-031. -->
<!-- Generated-family law pass (ADR-036, ratified Huayin 2026-08-20). Substantive changes, each marked inline: Ch 6.3/6.4 — the B-anchor crossing is REFUSED, not served-with-critical (supersedes ADR-020 for this case only; the clause "the B-anchor crossing remains serve-and-disclose" in the ADR-031 ledger above is retired). Ch 19.2 — the crossing moves from "Serve, with disclosures" to "Inform" (the wire's `refuse`), and the determinacy principle gains its lawfulness precondition. Appendix B (concordance) — two rows added, the fail-closed row amended. The check is on the OPERATION, not the leaf, so it reaches the written member, the inline generated reducer, and every unary/binary/scalar/scan/DERIVED carrier: generation creates identity, not permission. Applicability stays per operator × lineage — ADR-031 D5 clarified, not reversed; no stock/flow/rating type returns. -->
...
- **Applicability (the B-anchor)** answers *along which dimension families may this reducer legitimately coarsen?* It is a plan-time check: a reduction along a family in the B-anchor is **refused** (Chapter 6.4). It is what makes summing a stock across time impossible — not merely impossible to do silently. <!-- X1: ADR-020 --> <!-- ADR-036 (2026-08-20): was "served only with the critical crossing disclosure ... and withheld only under a declared WITHHOLD". -->
...
For an **aggregate** with reducer `g` applied to a column: identify which dimension families are being eliminated by the coarsening (dropped axes for a drop; the climbed family for a climb). For each eliminated family, check whether it is in the column's family-set entry for `g` — that is, whether `g`'s B-anchor for this column contains the family. If any eliminated family is in the B-anchor, the crossing fires: the operation is refused, per Chapter 6.4. <!-- X1: ADR-020 --> <!-- ADR-036 (2026-08-20): was "the result is served only with the critical disclosure of Chapter 6.4 (or withheld under a declared WITHHOLD)". A declared WITHHOLD remains a separate governance instrument over families the law would otherwise permit; it is no longer the only route to withholding a crossing. -->

For a **broadcast** refining the anchor: broadcast is replicate-only and needs no allocation, so there is no plan-time check at the broadcast itself. The B-anchor's role here is downstream — the broadcast records the family it spread along in the B-anchor of any reducer that would sum the result, so a later attempt to re-aggregate the broadcast value back across that family is caught by the ordinary coarsening check above and refused on the same grounds (the double-count guard of Chapter 2.4). <!-- ADR-036 (2026-08-20): "carries the same critical disclosure" → refused on the same grounds. -->
...
When the B-anchor check fails, the operation is caught at plan time and **refused**: no result is produced. The refusal names the column, the reducer, the eliminated family, and the violated B-anchor entry; quantifies the crossing where it can (the double-count or replication factor); and names the lawful edits — an alternative reducer from the column's family set applicable along the violated axis, where one exists, and an output grain the reduction does not have to cross. The declaration is what withholds, and it withholds in **every spelling of the operation**: whether the reducer was written as a declared family member, *generated* by an inline reducer above a lawful member of the same column, or carried inside a map, scan, or derived-column expression, it is the same reduction travelling the same barred lineage. An author's `WITHHOLD` on the family entry remains a separate instrument — governance over a family the law would otherwise permit, reported as the author's rule (Chapter 19). <!-- X1: ADR-020; ADR-025 -->

> **Supersession note (2026-08-20, ADR-036 — the generated-family law).** Until this ruling, this section read: *the result, if produced, is served only with a **critical disclosure*** — the crossing was a soundness finding, and "the crossing is never the engine's grounds for withholding." That reading is retired for the structurally prohibited reduction, and ADR-020's inform-and-serve doctrine is otherwise untouched. The rule now: **structurally prohibited analytical operations refuse. Disclose exists inside the lawful region; it cannot legalize an operation the governed law does not possess.** A caveat attached to a number that should never have been produced was the mechanism by which the same meaningless total kept being served, one wrapper at a time — *family generation creates a new analytical family; it does not create a new operator permission.* The disclosure code `b_anchor_crossing` is **tombstoned as a producer** and retained, still wired, so archived annotations and recorded transcripts still resolve; the Refuse channel's reason (`blocked_reduction`) carries the same spelling on the other channel. What did **not** change: applicability remains a fact about a `(column, reducer, family)` triple — per operator × lineage — so `SUM` of the same balance across customers stays lawful, and the column's other reducers across time stay lawful unless *they* are declared blocked. No stock/flow/rating type returns by this route (ADR-031 D5 clarified, not reversed).


A canonical example: `SUM(eom_inventory) @ {store}` from `{store, day}` drops the day axis. The column's `SUM` family has time in its B-anchor. The framework refuses the figure — "summing a daily stock over the period counts the same inventory once per day" — and notes that the column has a `LAST` family applicable along time. The refusal is informative, grounded in declared knowledge about *this* column's behavior, and it is not evaded by rephrasing: `SUM(LAST(eom_inventory) @ {day}) @ {store}` generates the same prohibited sum above a lawful member and refuses identically. <!-- X4: retail vocabulary --> <!-- ADR-036 (2026-08-20): was "serves the figure carrying the critical finding"; the closing clause ("an author who never wants the crossing served declares WITHHOLD") is dropped as moot — no author declaration is needed to withhold what the B-anchor already bars. -->
...
The line worth drawing runs along the **estimation boundary**, not through a single `impute` token, and it is a line between *kinds of behavior*, never between editions of the product. The shipped package supports the constant `fill` — the behavior whose correctness conditions are stated and checkable — and classifies missingness (MCAR / MAR / MNAR) from the declared M-anchor, disclosing the classification on the served result. **Rescale and stratify behaviors, and the sophisticated estimators beyond them** — model-based and multiple imputation, inverse-probability weighting, selection-model corrections — are **[ROADMAP]**. When they arrive, each enters as an additional named behavior in the operator registry (Chapter 8.3) that the missing-policy field may then invoke, not as values smuggled under an `impute` rule: the mechanism is uniform, and what grows is the catalog of registered behaviors. Until one is registered, an undeclared absence is disclosed rather than estimated — the system says it does not know, which is the only honest behavior a missing estimator can have.
...
#### 8.3 Extending the registry

The registry is the framework's one extension point, and it is not an edition boundary: *operators and missing-data behaviors* enter it by registration — sophisticated estimators (model-based and multiple imputation, inverse-probability weighting, selection-model corrections), custom domain-specific reducers, allocation rules for many-to-many bridges, all **[ROADMAP]**. That the mechanism is real and not aspirational is shown by the one family already built through it: the distinct family is a custom TYPE (`HLLSketch`) plus three registered custom operators (`hll_count` / `hll_merge` / `hll_estimate`), and the planner needed no change to route them. Each registration carries the same four fact-classes (algebraic signature, intensive/extensive bit, B-anchor propagation, aliases) and, for missing-data behaviors, named so the missing-policy field can select it. An estimator is a *named behavior* the missing-policy field invokes (`model_based(features=[...])`), registered alongside the five default behaviors `skip` / `skip-and-rescale` / `stratified` / `propagate` / `fill` — not a value smuggled under a generic `impute` token, which the v4 model removed, and not a withholding rule, which is governance (`WITHHOLD`), not a behavior (Chapter 7.5). <!-- ADR-025 -->

This is the principled extension point: the package ships the standard operators and the default missing-data behavior; further operators and estimators enter by registration; column-specs select among the registered options by naming them in their family entries. The mechanism is uniform. What differs between a build and a later one is the *catalog* — never the language, and never what a reader is permitted to have.
...
Drift is detected and recorded — that part ships. **No operational monitoring infrastructure exists**: nothing consumes those events, and there are no alerts, no dashboards and no escalation **[DELIVERY-OPERATIONS — unbuilt]**. The distinction is real and worth keeping, but it is a distinction between *kinds of machinery*, not between editions of a product: detection is correctness machinery, and the operational surfaces that would act on it are operations machinery. The line falls where data that has been verified becomes data that is acted on — and today the second side of that line is empty.
...
- **Custom functions and reducers** — extensions to the operator set specific to this Manifold **[ROADMAP — the registry is the extension point (Chapter 8.3); no authoring surface declares one]**.
...
<!-- X1: ADR-020 — chapter rewritten from the three-outcome (proceed/refuse/warn) model to the four-outcome inform-and-serve model; annotation contract aligned with the Frame-QL manual's Chapter 7. Behavior-changing rewrite; safety asymmetry preserved verbatim. --> <!-- ADR-036 (2026-08-20) — one outcome REASSIGNED within that taxonomy: the B-anchor crossing moves from "Serve, with disclosures" to "Inform" (the wire's `refuse`), because a structural prohibition is not an analytical risk. The four-outcome taxonomy itself, the annotation contract, and the safety asymmetry are unchanged; ADR-020's historical text stands and is not rewritten. The moods now sort by lawfulness: serve = lawful, no material condition; disclose = lawful, a material condition travels; clarify = several lawful meanings remain; refuse = no lawful path exists. -->
...
**Serve, with disclosures.** The query is executable, determinate, and *lawful*, and one or more soundness findings apply — a coverage gap, an approximate realization such as a sketch-backed distinct count, an MNAR exclusion, a contradicted precondition in the cone. *(2026-08-20, ADR-036: **the B-anchor crossing left this list** — it heads the one below. A soundness finding is a condition on an answer the law permits; a crossing is an operation the law does not permit, and there is no answer to attach a condition to.)* The result is served; the findings ride with it. This outcome has a first-class sub-case, **route-around**: where the planner can *dissolve* a risk rather than flag it — recomputing a mule from its fertile sources instead of re-aggregating a cached image; serving from transaction detail when a `CONTRADICTED` or stale pre-aggregate would otherwise be used — it does so and discloses the routing as an `info` finding (`RECOMPUTED_FROM_DETAIL`, `SERVED_FROM_DETAIL`) rather than disclosing a risk that no longer exists. Only when no sound route exists does the risk surface as a `caution` or `critical` finding on the served number.
...
The many-to-many case is worth stating precisely, because it is the boundary between this outcome and the one above, and the determinacy principle that draws it governs every outcome. Aggregating a measure *across* a non-functional edge — `revenue` at product grain summed to `category` over an M:N membership — is **fan-out**: the replicate-and-sum has no single target (a value at one product does not map to one category), so the request **denotes no single number at all**. It is therefore not a determinate-but-risky computation to be served with a disclosure, but an *underdetermined* one to be clarified: its legitimate readings — membership ("revenue touching each category"), primary designation, allocation **[ROADMAP]** — are different questions with different numbers (Chapter 22.5), and the framework names them rather than silently computing one. The principle, in its reconciled form: **a request that denotes exactly one number, and that the declared law permits, is served** (with a disclosure if that number is risky); **a request that denotes no single number among legitimate readings is clarified**, never guessed; **a request that denotes exactly one number the law does not grant is refused** — the B-anchor crossing is the canonical case of the third, and until 2026-08-20 it was cited here as the canonical case of the first. Determinacy is a precondition of serving, not a licence for it. *(ADR-036; Chapter 6.4.)* This supersedes the prior edition's classification of the bare aggregate-across as a served membership aggregation; membership is now served only when it is the *chosen* reading. <!-- ADR-031 D10: M:N aggregate-across reclassified from serve-with-disclosure (membership default) to Clarify, on the determinacy principle. Supersedes ADR-020/025 default-to-membership. The B-anchor crossing remains serve-and-disclose. --> <!-- ADR-036 (2026-08-20): the LAST CLAUSE of the D10 note above is superseded — the B-anchor crossing REFUSES. D10's own M:N reclassification stands. Three sortings, not two: no single number -> Clarify; one number the law does not grant -> Refuse; one number the law grants -> Serve, risk on its face. -->

**Inform.** (The shipped wire's fourth mood, `refuse`.) The query is well-formed but no result can or may be produced for a non-analytical reason: the **declared law does not grant the operation** — a reduction travelling a lineage its reducer is declared blocked along for that column, which has no lawful reading in any spelling (Chapter 6.4, shipped reason `blocked_reduction`); the data is not there (an empty realized support, disjoint filtered restrictions); a column is quarantined (the author's standing coverage governance); a declared `WITHHOLD` fires (the author's hard stop on a column or family entry, reported as the author's rule with its rationale and the permitted alternatives); or the caller is not entitled under the Manifold's access rules. The framework returns no result and states whose rule, why, and the remedy. *(2026-08-20, ADR-036: the first limb is new. It is a reading of the author's own B-anchor declaration, not an analytical judgment the framework formed — which is why it belongs in this outcome and not in a fifth one.)*

There is no fifth outcome, and in particular there is no analytical-risk withholding: any result that is determinate, lawful, and producible for an entitled caller is produced, with its risk on its face. The engine's only inherent non-serves are the three it structurally cannot do — execute an ambiguous instruction, execute an instruction the declared law does not authorize, or serve from nothing; everything else it serves and discloses, executing governance rules it did not author and producing disclosures it does not act on. *(2026-08-20, ADR-036: "the two" became "the three", and "determinate" gained "lawful". The added non-serve is still free of the engine's own analytical judgment — it enforces a declaration the author wrote, in the spellings the author did not anticipate as well as the one they did.)*
...
A clarification names the underdetermined choice in actionable terms: *"the inner grain of this mean is underdetermined — name the input anchor (`@ {customer}` and `@ {transaction}` give different answers)."* Or, for a non-functional aggregate-across: *"`revenue by category` crosses a many-to-many edge (`product → category`) — it has no single total; did you mean membership (revenue touching each category, totals overlap), a primary category (a true partition), or an allocation split [ROADMAP — not available in this build]?"*
...
- **Custom functions and reducers** **[ROADMAP]** — operator extensions specific to the Manifold; the registry is the extension point (Chapter 8.3).
...
**Bridge with allocation [ROADMAP]** when totals must reconcile. Declare the bridge, choose an allocation rule, accept that per-bucket numbers reflect the allocation's apportionment. No allocation operator ships: absent a primary designation, the aggregate-across stays a clarification — the planner names allocation as one of the readings and marks it unavailable in this build — rather than silently allocating. <!-- ADR-031 D6/D15: allocation is Pro. --> <!-- tier-history: that classification was retired 2026-08-31, topology record §17.5; the construct is typed [ROADMAP] in the body. -->
...
- **`FILL <rule>`** — the **compiled form of the measure's fill rule Φ**: what an absence at an eligible point of this column *denotes*. It is what a Manifold's `fill_rule` **lowers to**, not a constant an author hand-writes — authoring declares `Φ ∈ {zero, unknown, undefined}` on the *measure* (the M-contract clause; columna#143) and the lowering emits `FILL` here. `zero` is the nil of an event-derived quantity (nothing occurred); `unknown` marks a value that existed and was not recorded; `undefined` marks a point outside the population — and where Φ is undeclared the engine **discloses rather than fills**. A property of the measure, declared once and inherited by every member — and, since the universe basis no longer decides what an absence means (columna#148), the sole carrier of that decision (Chapter 22.1).
...
> **[`WITHHOLD`, `ACCESS`: ROADMAP]** — access governance requires an authentication model the package deliberately lacks; no keyword is scheduled (ADR-034 D3).
...
<!-- tier-history:start -->
> **▸ Currency repair, 2026-08-31.** This matrix previously carried four columns — `Polars (Core)`,
> `DuckDB (Core)` and `Pro connectors` — of which two named things that do not exist. **`PolarsConnector`
> appears in six documents and zero lines of code**, and `columna_core.connector` defines exactly one
> concrete connector, `DuckDBConnector`; the framework manual said so at the same moment this matrix
> contradicted it. And there is no second tier of connectors to vary (topology record §17.5). Both
> columns are struck.
<!-- tier-history:end -->
>
> A third column is **added**, and it is the one a reader actually needs. This matrix is the
> framework's *logical* catalog, and it was being read as a statement about the shipped package —
> understandably, since its headers said "Core". Resolving every operator class below through
> `columna_core.operators.get_operator` shows **fifteen of them do not resolve at all**: the shipped
> registry holds 26 operators, and `AVG`, `COUNT_DISTINCT`, `APPROX_DISTINCT`, `APPROX_QUANTILE`,
> `APPROX_FREQUENCY`, `PRODUCT`, `BOOL_OR`, `BOOL_AND`, `WEIGHTED_MEAN`, `VARIANCE`, `STDDEV`,
> `VALUE_AT_MAX/MIN`, `NTH`, `rank` and `ewm_mean` are not among them. Rather than silently delete a
> catalog that is legitimate *as a catalog*, the **registered** column now separates the two claims:
> what the framework defines, and what this build will accept from you today.
...
**Two axes, never one.** *Canonical standing* is what the language has ruled in (ADR-035 D1: the grammar grows by ruling). *Core Profile* is what a conforming implementation undertakes to realize. *This build* is measured from the installed `columna-core` (0.18.1). A profile may add realization capability; it may not change what canonical Frame-QL means. A build may conform to, lag, or exceed a profile; it does not redefine it.

### Reducers

| Capability | Canonical standing | Core Profile | This build |
|---|---|---|---|
| `all` | proposed | — | — |
| `any` | proposed | — | — |
| `approx_distinct`, `distinct` | ratified | executes | executes |
...
| `mean`, `avg` | ratified | executes | executes |
| `median` | ratified | executes | executes |
| `min` | ratified | executes | executes |
| `mode` | ratified | executes | executes |
| `nth` | proposed | — | — |
| `product` | proposed | — | — |
| `stddev` | proposed | — | — |
| `sum` | ratified | executes | executes |
...
| `month` | proposed | — | — |
| `*` | ratified | executes | executes |
| `neg`, `unary -` | ratified | executes | executes |
...
A canonical vocabulary governed by **syntax position**: these are usable in `WHERE` / `HAVING` and are not series-expression operators.

| Capability | Canonical standing | Core Profile | This build |
|---|---|---|---|
| `between` | proposed | — | — |
| `and` | ratified | executes | executes |
| `or` | proposed | — | — |
| `=`, `==` | ratified | executes | executes |
| `>=` | ratified | executes | executes |
| `>` | ratified | executes | executes |
...
| `impute(rule)` | **`fill`** for constants; estimation is `skip-and-rescale` / `stratified`; sophisticated estimators would enter as registered named behaviors **[ROADMAP]** | v4 model; Chapter 7.5 |
...
| fail-closed (as identity) | **inform-and-serve** — disclosure as identity; the safety asymmetry preserved verbatim; refusal survives for clarify / no-data / governance / authorization — and, from 2026-08-20, for the **structurally prohibited operation** | ADR-020; ADR-036 (2026-08-20); Chapter 19 |
...
| membership aggregation over an unresolved many-to-many (served with disclosure, ADR-020/025) | **Clarify** — the bare aggregate-across is underdetermined; membership is served only when *chosen*, and allocation is **[ROADMAP]** | ADR-031 D10; Chapters 19.2, 22.5 |
| B-anchor crossing served with a critical disclosure (ADR-020/025) | **Refuse** (`blocked_reduction`) — a structurally prohibited reduction has no lawful reading, in every spelling; *Disclose exists inside the lawful region and cannot legalize an operation the governed law does not possess* | ADR-036 (2026-08-20); Chapters 6.4, 19.2 |
| `b_anchor_crossing` (disclosure code) | **tombstoned as a producer**, retained and still wired — archived annotations and recorded transcripts resolve unchanged; the Refuse channel's reason carries the same spelling (one concept, two channels) | ADR-036 (2026-08-20); Chapter 6.4 |
