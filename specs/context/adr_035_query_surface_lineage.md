...
- **Growth candidates, durably queued — tier one: "engine-shipped, surface pending"** (the manuals-alignment WP, CP-M1 M1-d ruling, 2026-07-14). Clauses whose *capability already ships in the engine/planner* but which have no envelope-grammar surface: **`WHERE`** (pre-query input filtering — the planner carries it) and **the scan clauses** (`cumsum`/`rolling_*`/`lag`/`lead` with their order/`reset`/`step`/`within` parameters — carried today). They enter the grammar by ruling under D1 when a need is attached, each with its legality doctrine and tests; **this listing is not a scheduling promise**. It is distinct from genuinely unshipped semantics (`HAVING`, `LIMIT … PER`, `WITH` macros/allocation, bracket-filter), which are **[ROADMAP]** and are not D1 growth candidates. The Frame-QL manual's Chapter 6 marks these two tiers accordingly.
