...
1. **Code migration (the immediate next pass).** Shipped Core (v0.7.3) still raises a single `Refusal` for *both* clarify and refuse, and the column engine raises it directly in two paths — the multi-grain `_attr_anchor` case and the contradicted-edge path. Making D3/D4 real requires: (a) moving those refusals *out* of the engine into a no-result + disclosure handed up; (b) adding the ambiguous-vs-unsupported discriminator to the disclosure; (c) making clarify a structured *result* (carrying its alternatives) rather than an exception, so an MCP agent receives it as an actionable signal; (d) separating the four outcomes into distinct wire kinds. Scope as its own pass against v0.7.3.
