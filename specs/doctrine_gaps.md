...
| ~~DG-2~~ | 2026-07-16 (CP-1 §2c, [PR #35](https://github.com/datumwise/columna/pull/35)) | The everything-classifies guarantee: every query resolves to a classified outcome (serve/disclose/clarify/refuse/error), never a raw exception. `level.sum @ cal.month` (a stock whose `sum` is BLOCKED over calendar) collapsed to `cal.month` — collapse a base coordinate (`store`) while transporting another (`day`) across the blocked lineage — SHOULD serve with a critical `blocked_reduction` caveat (as `level.sum @ store*cal.month` does, §2a). | On main it leaked a raw `ColumnNotFoundError` ("unable to find column 'store'") — collapse-while-blocked-transport escapes classification entirely (verified pre-existing, not a §2c regression; found by Huayin's probe). This WP adds an everything-classifies BACKSTOP in `planner.run` (raw exception → classified ERROR/`unsupported`), so it no longer ships unclassified — but it now ERRORS where it should SERVE-with-caveat. | The engine's collapse path drops the collapsed base coordinate's column before the blocked-lineage transport annotates it; the structural fix is engine-side (`engine.py`), out of §2c's planner scope. | ~~**CLOSED** 2026-08-20 by the generated-family law correction. `level.sum @ cal.month` now REFUSES (`blocked_reduction`), not ERRORs — and not with the caveat the original row asked for, because the 2026-08-19 disposition retired that target. The everything-classifies backstop stays as a backstop; this case no longer reaches it. Pinned by `test_dg2_collapse_with_blocked_transport_refuses` and by the Afternoon matrix (`test_generated_family_law.py`, 55 cases), which certifies forward invariant 5 against a real fixture for the first time. **Evidence pointer corrected 2026-08-20:** invariant 5's evidence is that matrix and nothing else. `scripts/afternoon_five.py` was repurposed the same day into the LITERAL *Theory of Data in One Afternoon* v0.12 page gate (the five executable statements the essay prints, run through the statement parser, with explicit `--engine source|installed` provenance), mirrored in CI by `test_afternoon_page_gate.py`. That gate certifies the ESSAY; the matrix certifies the LAW. Do not cite the script for invariant 5.~~ |

| DG-3 | 2026-08-20 (generated-family law) | A DECLARED derived successor family should travel only where its declaration establishes it. Ruling 2026-08-20 §1 defers the whole question rather than forcing it through `FERTILE`, on the boundary distinction: **family law ≠ certification evidence ≠ runtime admission.** | Nothing consults the derived family's declared law at runtime, so `FERTILE { calendar }` and `FERTILE { }` are behaviourally identical — both travel and serve clean. Unchanged by this correction, deliberately. | `FERTILE` cannot carry the meaning. It is an EQUALITY THEOREM about the reduce-path — "reducing from cached finer values equals recomputing from base" — adjudicated against attested data by `adjudication._prove_data`, i.e. it sits on the CERTIFICATION-EVIDENCE boundary. An AT-metric's travel is the opposite of that by construction: `daily_aov AT day` reduced to month is the mean of daily rates, deliberately NOT the recompute value (pinned by `test_at_metric_at_coarse_is_mean_of_finer_not_pooled`). Measured: `mean FERTILE { calendar }` fails publish closed with a Contradiction (reduce=131.063… vs recompute=129.787… @ `cal.week=2024-W01`); `FERTILE { }` would forbid the metric's own declared meaning and reverse ruling 5 (`engine.py`: "infertility bars silent reduction, never reduction that is the declared meaning"). There is NO declaration an author could write to permit the travel. | **OPEN** — deferred WHOLE per ruling §1: no widened `DerivedShape`, no unreachable adjudication branch, no half-registered reason. Do not solve this by treating the current License as family identity or family law. Needs a dedicated reconciliation of the three boundaries; a standing test (`test_no_engine_refusal_reason_is_unregistered`) will catch any reason minted for it that is not also registered. |
...
| DG-6 | 2026-08-20 (generated-family law) | A successor family produced by anchor-preserving MAP composition should carry the law its composition actually earns. A ratio of two non-reconciling quantities can be perfectly reconciling; a difference of two blocked sums is still blocked. | The planner preserves the UNION of the operands' restrictions and offers no way to establish otherwise. Over-strict in the ratio case: `stock.sum / stock.sum` inherits the temporal-sum bar from both parents even though the quotient may reconcile. | Ruled acceptable for now (2026-08-20 §6) on the asymmetry that a conservative rule may withhold a lawful continuation but can never manufacture an unlawful one. Closing it needs a general law-synthesis mechanism — a way for a map composition to POSITIVELY establish a successor law — which is a declaration-surface question, not a planner one. The machinery that would inform it exists (`adjudication._homogeneous_linear` already reasons per-operator over operands with the scalar rule) but runs only at DERIVED publish. | **OPEN** — recorded explicitly per the ruling, not fixed. The governed mechanism available to authors today is an explicitly declared DERIVED/FAMILY path. |
...
| CT-2 | 2026-08-19 | **Analytical Governance v1.1 alignment.** A revision of the category paper was under consideration. | **CLOSED 2026-08-21.** v1.1 is published and is the publication of record — DOI `10.5281/zenodo.22046037`, 21 Aug 2026 — and the registry carries it as the current record of `w-analytical-governance`, superseding `w-analytical-governance.r01` (v1.0, DOI `10.5281/zenodo.21959749`), which remains a first-class historical record. No page was edited to effect this: `/about`, the `llms.txt` inventory and the `/analytical-governance` citation are registry-derived and re-derived on the next build. The row that said Slice 1 "does not anticipate the revision" is why this one exists — anticipating revisions is no longer a thing any surface has to do. |
