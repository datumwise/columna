...
| AW-3 | 2026-07-25 | Swap the site contact string to `contact@datumwise.ai` once the alias exists. | **Irena** (alias) → agent (one-line swap) | **OPEN** | `about.astro:7` carries the standing note; the value is already a single swappable const. Blocked on the alias existing. |
| AW-4 | 2026-07-25 | Retire / re-point `GITHUB_TOKEN` so exactly one variable, one identity, one scope does push work. | **Irena** (env sitting) | **OPEN** | Probe 2026-07-25: `GITHUB_TOKEN` → `reeeneeee`, **no** `contents:write`; `DATUMWISE_PUSH_PAT` → `datumwise`, `contents:write`. Standing rule: use the PAT for `datumwise/columna` work. |
| AW-5 | 2026-07-25 | The core README grammar correction reaches PyPI. | rides next release | **OPEN** | Corrected in-repo by [PR #90](https://github.com/datumwise/columna/pull/90). A README ships to PyPI only with a release, so it rides 0.12.2-whenever. **No release minted for prose** (ruled). |
