# D2 — Certificate Cargo Schema v0.3
### urn:columna:certificate:v1 — what rides the wire when a certified plan travels

*v0.3 · 2026-08-01 · cut on beat 3's inputs (#130). Changelog: (1) S7
SETTLED against its first real exhibits (C3 touch + the alloc stretch);
(2) §4b band mechanics CONFIRMED by evidence — same rule_id, different
digest across backend bands (02ed755e→9e0f68d1), identity-addressing
working as designed; band WIDTH stays open for v0.4; (3) M3 gains
`retries` under the execution-error razor: retry licensed for the
execution-error class only, NEVER for a comparison mismatch — a
mismatch that retries is a silent failure of the harness itself.
Prior (v0.2) adjudication note: · 2026-07-31.
...
- **S7 `face_spends`** — SETTLED at v0.3, one entry per CROSS:
  `{face_id, scheme (touch|assign|alloc), disclosure_kind,
  reconciliation?}` — where `disclosure_kind` names the semantic
  disclosure the spend obligates (`multi_counted` for touch;
  `memberships_unrepresented` for assign) and `reconciliation` is
  present iff scheme=alloc, carrying the shipped badge shape
  `{crossed_total, base_total, delta, tolerance, status}`. Exhibits:
  C3's touch spend (disclosure minted at home, delivered across the
  seam) and the alloc stretch (base = crossed to 2.3e-9). CROSS
  execution still does not lower (D1); a stay-home spend feeding
  lowered work is precisely what S7 records.
...
Settled through v0.3: the channel-assignment correction; S6's list
shape; S5 ref-vs-embed; S9 notation; digest algorithm; S7's shape
(against C3 + the alloc stretch); band identity-addressing (evidence:
two consumers, same rule_id, distinct digests); the execution-error
retry razor (M3.retries — never for mismatches). Still open, carried
to v0.4: band WIDTH and re-proof triggers (needs version-spread
evidence); S8 severity grades; S7 for future face schemes. v0.3's
acceptance inherits v0.2's passed test and adds C3's native
conformant emission as the CROSS exhibit.
