...
| **GAP-1** | ~~attestation gap~~ **RESOLVED** (2026-07-31, amendment) | CARVE · WHERE→`FilterRel` | root cause: the query `WHERE` is raw-SQL-passthrough to duckdb, so a FrameQL double-quoted literal became an identifier and errored; **single-quoted SQL literals serve**. Two `WHERE` asks minted into the trace corpus; CARVE now attested `scan` + `scan+filter` (pushdown) + `filter` (`_confine`); verdict finalized **PR**, no asterisk |
