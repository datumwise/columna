# Open Planner — Artifact A1: the extracted Plan IR + the Cascadia attack set (v0.1)

*Desk artifact, 2026-07-27, delivered under the artifact gate. Method:
...
The model, as shipped: universes `transaction` (customer·store·product·day),
`inventory` (store·product·day), `category_profile`; `revenue FROM sales AS
sum(amount)` on transaction; `stock_units` on inventory; the
product↔category crossing with faces `touch` / `primary` (ASSIGN BY priority)
/ `split` (ALLOC BY alloc_weight); `location: store → region`.

**Attack A — lawful substitution (establishes lawfulness ⊅ faithfulness).**
ASK: `revenue.sum @ category` (face: split). PLAN: nodes CARVE(inventory) ·
COLUMN(stock_units, sum) · CROSS(product↔category, split) · REDUCE(@
category). Every node individually lawful — the plan is a perfect answer to
a *different servable ask*. Kernel obligation exposed: the faithfulness
check must bind `COLUMN.measure_ref` to the ask's measure atom — trivial
here, which is the point: Class A is the base case any obligation language
must kill in one rule.
