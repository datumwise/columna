> **Filing annotation — 2026-07-30 (filed into `specs/open_planner/`). Supersession + staleness catch.**
>
> This v0.11 fork document supersedes **v0.8**, now moved to `superseded/`; v0.11 governs. The
> published program note is at **v1.3** (`open_planner_deposit_v1_3.md`, DOI 10.5281/zenodo.21632723).
>
> Two `columna-core` landings postdate this document's assumptions. They are recorded here, not
> edited into the body — **history is annotated, never rewritten**:
>
> - **0.13.4 (WP-GRAIN-1) lifted F1's restriction.** The single-level restriction on inline-reduction
>   input anchors is gone — the composite input anchor (`@ {a*b}`) now serves. Where this document
>   treats F1 as a live parser-surface recall, read it as historical: F1 is resolved (OF-25 RULED (a)
>   & LANDED; `attack_b.py`).
> - **0.14.0 (WP-NAME-1) retired mechanical column identity.** An unaliased series is now keyed by its
>   **canonical expression** (or an `AS` alias); the mechanical `<reducer>_<measure>` default and the
>   dot-mangle are retired, and wire `contract_version` moved `"1"` → `"2"`. Where this document
>   assumes the mechanical-default naming, read it against canonical-expression identity (OF-27 RULED
>   (a) & LANDED).
>
> The program's argument stands; the specific mechanics these two points name have moved. — the desk
>
> ---

# The Open Planner — kernel and searcher (fork document v0.11)

*datumwise desk artifact, 2026-07-27, v0.11, 2026-07-29 (THE RULING + THE OUTBOUND TURN: fork ruled same day it was formalized; program redirected). v0.10 formalized the fork; beat 1 and rounds 8–9 folded at v0.9. Deposit published through v1.3 (2026-07-29). DOI 10.5281/zenodo.21632723.
...
transport: the file channel, fixed. Beat 1: COMPLETE — the artifact gate is SATISFIED. Fork RULED at v0.11;
beat 2 = the outbound mapping study (§10).** RG stands by — no search regions currently open; the loop
