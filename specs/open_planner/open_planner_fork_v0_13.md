# The Open Planner — kernel and searcher (fork document v0.13)

*datumwise desk artifact, 2026-07-27, v0.13, 2026-08-01 (BEAT 3 COMPLETE — both seams held: the vertical split verified not asserted, the horizontal transfer 2/2 with the amortization number measured; closure pending ratifier word). v0.12 closed beat 2. v0.11 carried the ruling + the turn; beat 1 and rounds 8–9 at v0.9. Deposit at v1.3; theory published (10.5281/zenodo.21696104 v1.1 + 10.5281/zenodo.21707018). DOI 10.5281/zenodo.21632723.
...
## 12 · BEAT 3 — THE TWO SEAMS (complete; closure pending ratifier)

**Chartered 2026-08-01 (map2_beat3_charter v0.1→ratified); executed in
PR #130; schema v0.3 cut on its inputs.**

**Experiment A — the vertical seam (C3): ACCEPTED.**
`SELECT revenue AT {category.touch}` — home CROSS (bridge
join-multiply + multi_counted minting) feeding a substrate sum on
Acero. A-1: conservation-clean end-to-end — final 12/12 vs the
all-home oracle, handoff 870/870 vs independent reconstruction
(N=882); both seam break modes fail loudly. A-3: **the split verified,
not asserted** — the substrate plan is ['aggregate','read'], ZERO
JoinRel: the bridge never left home, checked. A-4: the multi_counted
disclosure, minted at home, arrives in the answer with half the plan
on Acero — the custody law's observable half, observed. **The alloc
stretch PASSED**: {category.split} reconciles across the seam (base =
crossed = 2,212,391.86, delta 2.3e-9), oracle 12/12 — conservation
arithmetic survives the boundary. **Measured boundary (OF-26)**: a
single faced coordinate is the maximal expressible CROSS seam at v1;
every richer shape refuses chained_crossing (G4). A lowered transport
under a cross awaits chained-crossing licensing (future WP). The
handoff wall held — no improvised transport.

**Experiment B — the horizontal seam: 2/2 TRANSFER.** duckdb==1.1.3 +
substrait ext (study venv only): both rule certificates PASS on the
second consumer (worst 4.4e-11 sum, 9.1e-13 mean), minting DuckDB-band
siblings — same rule_id, different digest (02ed755e→9e0f68d1):
identity-addressing working as designed. **Economics measured (B-3):
consumer #2 certified in ~0.2s wall-clock per rule** — a harness run,
not a connector project, now a datum. **No drift specimen** — both
rules live where engines agree; drift risk concentrates where D1's
NOT-LOWERABLE verdicts already sit.

**The surprise, characterized and razored**: B-1's mean cell first
returned an execution ERROR (transient DuckDB-ext flake under
subprocess) — not drift (identical bytes, clean 4/4 on re-execution).
Fixed with a bounded retry, and the razor is now law: **retry is
licensed for the execution-error class only; a comparison mismatch may
NEVER retry** — with a negative test (a planted mismatch must fail
without a second chance).

**D5**: the G4 boundary (OF-26); the ext flake (fixed + razored); the
1.1.3 study-venv deployment question (MAP-2(a)'s, not this beat's);
band width (schema v0.4 trigger); the no-drift note.

**State**: charter §4 met on all five items — C3's conformant
certificate, B's verdict matrix (no third state), schema v0.3 cut,
D5 filed, this fold. The two boundaries the architecture stands on
both held under test in the same week they were named.
