# datumwise.ai — the argument (formerly the homepage)
**v0.3 (homepage v2 graduation, 2026-07-25) · GRADUATES to the route `/the-argument`. The homepage is
now a lobby that describes and points (see the homepage v2 copy spec); this essay — the boldness — moves
here intact and keeps the lineage. Section order and prose unchanged except its FRAMING and its ENDS:
the title is now "The Argument" with a deck; §1's old H1 becomes the essay's opening line; §8 "three
doors" RETIRES (the homepage directory supersedes it); the essay closes on the ratified line. copy is
verbatim-intent; [bracketed] blocks are design/build annotations.**

---

[AMBIENT CHROME — the shared site chrome]
- Masthead + primary nav are the site's (nav now gains **Positions**). The persistent bottom-right
  `pip install columna` chip is the site-wide ambient affordance.
- No hero image, no illustration on this route. Type, tables, and wire JSON only.
- This is a DESTINATION now, not a landing: §1 no longer takes the full viewport — the essay opens
  like a document (title, deck, first line) because the reader chose to be here.

---

## §1 — The opening line

[Title, then deck, then the essay's opening line — no longer a full-viewport hero.]

# The Argument

*The case for Columna, top to bottom — about eight minutes, with two live exhibits along the way.*
...
## Closing — the ratified line

[The "three doors" section RETIRES: the homepage directory (homepage v2 §5) supersedes it, so the
essay ends on its own argument, not a menu.]

**That is the argument. The demo is two minutes: `pip install columna`.** → [/install]
