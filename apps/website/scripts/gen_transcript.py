...
# The HERO MARK (redesign slice 1). The homepage's paper register quotes exactly one formal object,
# and a quotation from the wire has to be a real request the shipped engine actually answers — the
# hero teaches the address system before any notation is introduced, so it must not teach a retired
# syntax. Executed here and asserted to SERVE; the page renders this string from the generated
# block, never from hand-written homepage syntax. (The terse `measure @ {…}` form was retired in
# 0.9.0, which is precisely why this is generated rather than typed into an .astro file.)
HERO_MARK  = "SELECT revenue AT {store, cal.month}"
...
def main() -> int:
    ...
    # hero mark: the one formal object the homepage quotes in the paper register (see HERO_MARK)
    hero_wire = run(store, HERO_MARK)
    if hero_wire.get("outcome") != "serve":
        print(f"INTEGRITY FAIL: hero mark {HERO_MARK!r} no longer serves "
              f"(got {hero_wire.get('outcome')!r}) — the homepage must not quote a request the "
              f"shipped package cannot answer", file=sys.stderr)
        return 1
    ...
    out = {
        ...
        "seeded": seeded,
        "hero_mark": {"frameql": HERO_MARK, "outcome": hero_wire.get("outcome")},
